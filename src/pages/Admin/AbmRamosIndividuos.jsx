...
export default function AbmRamosIndividuos() {
  ...
  const { updateStatusBranch, currentBranch, setUpCurrentBranch, branches } =
    useBranchContext();
  //   const [checked, setChecked] = React.useState(true);

  const handleChange = (event) => {
    console.log(event.target.id, event.target.checked);
    const newbranch = branches.find((ramo) => ramo._id === event.target.id);
    setUpCurrentBranch(newbranch);
    updateStatus(event.target.id, event.target.checked);

    // updateStatusBranch(event.target.id, !event.target.checked);
  };

  const updateStatus = (id, status) => {
    updateStatusBranch(id, status);
  };
          ...
          >
            ...
            <TableBody>
              ...
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  const isItemSelected = isSelected(row.ramo);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.ramo)}
                      role="checkbox"
